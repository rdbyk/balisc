...
Function::ReturnValue sci_inner(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    InternalType* value = (in.size() == 2) ? in[1] : ctx->get(symbol);
    
    if (value)
    {
        Variable* var = ctx->getOrCreate(symbol);
        if (ctx->getScopeLevel() != ctx->getLevel(symbol))
        {
            var->pop();
        }
        ctx->put(var, value);
