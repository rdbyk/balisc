...
#define XPATH "//classpaths/path[not(@disableUnderMode='%s')]"
            ...
            if (xpathObj && xpathObj->nodesetval->nodeMax)
            {
                ...
                for (i = 0; i < xpathObj->nodesetval->nodeNr; i++)
                {
                    ...
                    if ( (classpath) && (classpath[0] != '\0') && (strncmp(classpath, "@", 1) != 0) ) /* If it starts by a @ that means it hasn't been able to find it... which is normal... for example with the documentation */
                    {
