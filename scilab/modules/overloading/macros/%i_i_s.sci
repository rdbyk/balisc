...
function M=%i_i_s(varargin)
    ...
    index=varargin(1)
    if nargin==3&(type(index)==10|type(index)==15) then //struct or cell creation
    ...
    elseif nargin>4 then//more than 2 indices: insertion of an integer in a matrix of numbers
        M=varargin($)
        if M<>[] then
