...
function s = %lss_i_lss(i,j,s1,s2)
    ...
    jj=zeros(1,p2);jj(j)=ones(1,prod(size(j)));
    ii=zeros(1,m2);ii(i)=ones(1,prod(size(i)));
    s=diag(ones(ii)-ii)*s2*diag(jj)+s2*diag(ones(jj)-jj)
    ii1=zeros(m2,m1);ii1(m2-m1+1:m2,:)=eye(m1,m1)
    ii1=ii1([find(ii==0),i],:)
    jj1=zeros(p1,p2);jj1(:,p2-p1+1:p2)=eye(p1,p1)
    jj1=jj1(:,[find(jj==0),j])
    s=s+ii1*s1*jj1
    s=minss(s)
