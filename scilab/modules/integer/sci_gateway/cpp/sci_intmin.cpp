...
static std::map<std::wstring, std::pair<InternalType::ScilabType, signed long long>> int_min = {
...
Function::ReturnValue sci_intmin(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    std::wstring in_typestr = L"int32"; // default
    ...
    auto found = int_min.find(in_typestr);

    if (found == int_min.end())
    {
        if (in[0]->isString())
        {
