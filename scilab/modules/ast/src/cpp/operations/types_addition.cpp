...
template<>
InternalType* add_S_S<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pL->getRef() > 0 ? (_pR->getRef() > 0 ? new Double(0) : _pR) : _pL;
    add(_pL->getFirst(), _pR->getFirst(), pOut->get());
    return pOut;
}

template<>
InternalType* add_S_SC<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pR->getRef() > 0 ? new Double(0.0, 0.0) : _pR;
    add(_pL->get(), 1, _pR->getFirst(), _pR->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}

template<>
InternalType* add_SC_SC<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pL->getRef() > 0 ? (_pR->getRef() > 0 ? new Double(0.0, 0.0) : _pR) : _pL;
    add(_pL->getFirst(), _pL->getImgFirst(), _pR->getFirst(), _pR->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}

// specializations for Double (Matrix + Scalar)
template<>
InternalType* add_M_S<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pL->getRef() > 0 ? new Double(_pL->getDims(), _pL->getDimsArray()) : _pL;
    add(_pL->get(), (size_t)_pL->getSize(), _pR->getFirst(), pOut->get());
    return pOut;
}

template<>
InternalType* add_MC_S<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pL->getRef() > 0 ? new Double(_pL->getDims(), _pL->getDimsArray(), true) : _pL;
    add(_pL->get(), _pL->getImg(), (size_t)_pL->getSize(), _pR->getFirst(), pOut->get(), pOut->getImg());
    return pOut;
}

template<>
InternalType* add_MC_SC<Double, Double, Double>(Double *_pL, Double *_pR)
{
    Double* pOut = _pL->getRef() > 0 ? new Double(_pL->getDims(), _pL->getDimsArray(), true) : _pL;
    add(_pL->get(), _pL->getImg(), (size_t)_pL->getSize(), _pR->getFirst(), _pR->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}

// specialization for Double (Matrix + Matrix)
template<>
InternalType* add_M_M<Double, Double, Double>(Double *_pL, Double *_pR)
{
...
template<>
InternalType* add_M_MC<Double, Double, Double>(Double *_pL, Double *_pR)
{
    int iDimsL = _pL->getDims();
    int iDimsR = _pR->getDims();
...
template<>
InternalType* add_MC_MC<Double, Double, Double>(Double *_pL, Double *_pR)
{
    int iDimsL = _pL->getDims();
    int iDimsR = _pR->getDims();
...
int AddSparseToDouble(Sparse* sp, Double* d, GenericType** pDRes)
{
    ...
    if (d->isIdentity())
    {
        ...
        if (pS->isComplex())
        {
            ...
            {
                pS->set(i, i, std::complex<double>(d->getFirst(), d->getImgFirst()), false);
            }
        }
        else
        {
    ...
    if (sp->isScalar() && d->isScalar())
    {
        ...
        {
            pRes->set(0, pRes->getFirst() + sp->getReal(0));
        }
    ...
    if (d->isScalar())
    {
        ...
        double dblVal = d->getFirst();
        C2F(dset)(&iSize, &dblVal, pRes->get(), &iOne);
        if (bComplex2)
        {
            double dblValI = d->getImgFirst();
...
InternalType* add_M_SC(T *_pL, U *_pR)
{
    ...
    add(_pL->get(), (size_t)_pL->getSize(), _pR->getFirst(), _pR->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}
...
InternalType* add_IC_M(T *_pL, U *_pR)
{
    ...
    for (int i = 0 ; i < iLeadDims ; ++i)
    {
        ...
        int index = _pR->getIndex(piIndex);
        add(_pR->get() + index, 1, _pL->getFirst(), _pL->getImgFirst(), pOut->get() + index, pOut->getImg() + index);
    }
...
InternalType* add_IC_MC(T *_pL, U *_pR)
{
    ...
    for (int i = 0 ; i < iLeadDims ; ++i)
    {
        ...
        add(_pL->getFirst(), _pL->getImgFirst(), _pR->get(index), _pR->getImg(index), pOut->get() + index, pOut->getImg() + index);
    }
...
{
    O* pOut = new O(0.0, 0.0);
    add( _pR->get(), 1, _pL->getFirst(), _pL->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}
...
{
    O* pOut = new O(0.0, 0.0);
    add(_pL->get(), _pL->getImg(), 1, _pR->getFirst(), _pR->getImgFirst(), pOut->get(), pOut->getImg());
    return pOut;
}
...
InternalType* add_M_M<String, String, String>(String* _pL, String* _pR)
{
    ...
    else
    {
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            if (pwstL == String::nullValue())
            {
                pOut->get()[i] = (wchar_t*)MALLOC((sizeL + sizeR + 1) * sizeof(wchar_t));
                wcscpy(pOut->get(i), pwstR);
            }
            else
            {
...
InternalType* add_S_S<String, String, String>(String* _pL, String* _pR)
{
    ...
    else
    {
        ...
        if (pwstL == String::nullValue())
        {
            pOut->get()[0] = (wchar_t*)MALLOC(sizeOut * sizeof(wchar_t));
            wcscpy(*pOut->get(), pwstR);
        }
        else
        {
...
template<> InternalType* add_M_M<Polynom, Polynom, Polynom>(Polynom* _pL, Polynom* _pR)
{
    ...
    if (_pL->isScalar())
    {
        ...
        //Result P1(0) + P2(i)
        SinglePoly *pCoef1 = _pL->getFirst();
    ...
    if (_pR->isScalar())
    {
        ...
        //Result P1(i) + P2(0)
        SinglePoly *pCoef2 = _pR->getFirst();
        double *p2R = pCoef2->get();
        double *p2I = pCoef2->getImg();
...
template<> InternalType* add_M_I<Polynom, Double, Polynom>(Polynom* _pL, Double* _pR)
{
    ...
    if (isComplex)
    {
        ...
        for (int i = 0 ; i < iLeadDims ; ++i)
        {
            ...
            add(dblR, dblI, pSP[index]->getFirst(), pSP[index]->getImgFirst(), pSP[index]->get(), pSP[index]->getImg());
        }
    }
    else
    {
        ...
        for (int i = 0 ; i < iLeadDims ; ++i)
        {
            ...
            int index = _pL->getIndex(piIndex);
            add(dblR, pSP[index]->getFirst(), pSP[index]->get());
        }
    }
...
template<> InternalType* add_M_M<Sparse, Double, Double>(Sparse* _pL, Double* _pR)
{
    ...
    if (_pR->isScalar())
    {
        ...
        if (bComplex2)
        {
            double dblValI = _pR->getImgFirst();
