...
InternalType* SingleStruct::insert(typed_list* _pArgs, InternalType* _pSource)
{
    ...
    if (_pArgs->size() != 1)
    {
        throw ast::InternalError(_W("Unable to insert multiple item in a struct.\n"));
    }

    if ((*_pArgs)[0]->isString() == false)
    {
        throw ast::InternalError(_W("Assignment between unlike types is not allowed.\n"));
    }
...
bool SingleStruct::toString(std::wostringstream& ostr)
{
    ...
    else
    {
        ...
        for (auto & p : m_wstFields)
        {
            ostr << p.first << L" : " << m_Data[p.second]->getTypeStr() << L"\n";
        }
    }
