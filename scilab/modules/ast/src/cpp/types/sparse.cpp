...
void Sparse::set(int _iRows, int _iCols, std::complex<double> v, bool _bFinalize)
{
    if (matrixReal)
    {
...
void Sparse::set(int _iRows, int _iCols, double _dblReal, bool _bFinalize)
{
    if (matrixReal)
    {
        if (matrixReal->isCompressed() && matrixReal->coeff(_iRows, _iCols) == 0)
...
void Sparse::append(int r, int c, types::Sparse SPARSE_CONST* src)
{
...
SparseBool* Sparse::newNotEqualTo(Sparse &r)
{
    ...
    if (isComplex() || r.isComplex())
    {
        toComplex();
        r.toComplex();

        CplxSparse_t* ll = matrixCplx;
        CplxSparse_t* rr = r.matrixCplx;
...
void SparseBool::append(int r, int c, SparseBool SPARSE_CONST* src)
{
...
void SparseBool::set(int _iRows, int _iCols, bool _bVal, bool _bFinalize) SPARSE_CONST
{
