...
bool Bool::subMatrixToString(std::wostringstream& ostr, int* _piDims, int /*_iDims*/)
{
    ...
    else if (getRows() == 1)
    {
        ...
        if (iLastVal != 0)
        {
            ostr << L"\n       column " << iLastVal + 1 << L" to " << getCols();
        }
        ostr << L"\n\n " << ostemp.str() << std::endl;
    }
    else
    {
            ...
            if (iLen + 2 > iLineLen)
            {
                ...
                for (int iRows2 = m_iRows2PrintState ; iRows2 < getRows() ; iRows2++)
                {
                              ...
                              (iMaxLines != 0 && iCurrentLine + 1 >= iMaxLines && iRows2 != m_iRows2PrintState)))
                    {
                        ...
                        {
                            //add header
                            ostr << L"\n       column " << iLastCol + 1 << L" to " << iCols1 << L"\n\n";
                        }
                        ostr << L" " << ostemp.str();
        ...
        for (int iRows2 = m_iRows2PrintState ; iRows2 < getRows() ; iRows2++)
        {
            ...
            if ((iMaxLines == 0 && iCurrentLine >= MAX_LINES) || (iMaxLines != 0 && iCurrentLine >= iMaxLines))
            {
                ...
                {
                    //add header
                    ostr << L"\n       column " << iLastCol + 1 << L" to " << getCols() << L"\n\n";
                }
...
std::wstring Bool::toStringInLine()
{
    if (isScalar())
    {
        return getFirst() ? L"T" : L"F";
    }
    else
    {
