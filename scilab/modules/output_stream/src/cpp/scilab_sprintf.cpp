    ...
    *_piNewLine = 0;
    ...
    while (finish == false)
    {
        ...
        if (end > 0)
        {
            ...
            {
                wchar_t* pwstAfterInt;
                int index = static_cast<int>(wcstol(pwstStart + 1, &pwstAfterInt, 10));
                if (index > 0 && pwstAfterInt == pwstDollar)
                {
                ...
                case types::InternalType::ScilabDouble:
                {
                    ...
                    if (std::isfinite(dblVal))
                    {
                        if (tok->widthStar)
                        {
                            if (tok->precStar)
                            {
                                fwprintf(fwstTemp, token, tok->width, tok->prec, dblVal);
                            }
                            else
                            {
                                fwprintf(fwstTemp, token, tok->width, dblVal);
                            }
                        }
                        else
                        {
                            ...
                            {
                                fwprintf(fwstTemp, token, dblVal);
                            }
                        }
                    }
                    else
                ...
                case types::InternalType::ScilabInt64:
                {
                    ...
                    if (std::isfinite(dblVal))
                    {
                        ...
                        if (tok->widthStar)
                        {
                            if (tok->precStar)
                            {
                                fwprintf(fwstTemp, token, tok->width, tok->prec, iVal);
                            }
                            else
                            {
                                fwprintf(fwstTemp, token, tok->width, iVal);
                            }
                        }
                        else
                        {
                            if (tok->precStar)
                            {
                                fwprintf(fwstTemp, token, tok->prec, iVal);
                            }
                            else
                            {
                                fwprintf(fwstTemp, token, iVal);
                            }
                        }
                    }
                    else
