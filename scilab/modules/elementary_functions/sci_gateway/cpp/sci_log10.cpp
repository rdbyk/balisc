...
types::Function::ReturnValue sci_log10(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    if (pDblIn->isComplex())
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            std::complex<double> lz(std::log10(std::complex<double>(InR, InI)));
            pOutR[i] = lz.real();
            pOutI[i] = lz.imag();
        }
    }
    else
    {
        ...
        if (bIsLessZero)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                //std::complex<double> lz(std::log10(std::complex<double>(pInR[i], 0.0)));
                //pOutR[i] = lz.real();
                //pOutI[i] = lz.imag();
            }
        }
